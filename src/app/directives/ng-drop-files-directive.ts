...
export class NgDropFilesDirective {
    @Input() public files: Array<AssetObj> = [];
    @Output() public fileOver: EventEmitter<any> = new EventEmitter();
    @Output() public onFileDrop: EventEmitter<AssetObj[]> = new EventEmitter<AssetObj[]>();
  ...
  private _addFiles(files: FileList):void {
    _.each(files, (file) => { if (this._fileCanBeAdded(file)) this.files.push(new AssetObj(file)) });
  }
